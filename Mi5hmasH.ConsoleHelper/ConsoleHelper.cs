...
namespace Mi5hmasH.ConsoleHelper;
...
public static partial class ConsoleHelper
{
    ...
    /// <summary>
    /// Parses a command-line argument string into an array of individual arguments.
    /// </summary>
    /// <param name="args">A single string containing command-line arguments, which may include quoted values and spaces.</param>
    /// <returns>An array of strings, each representing a parsed argument from the input. Quotation marks are removed from quoted arguments.</returns>
    public static string[] GetArgs(string args)
    {
        var matches = ConsoleArgsRegex().Matches(args);
        return [..matches.Select(m => m.Value.Trim('"'))];
    }
    [GeneratedRegex("""[\"].+?[\"]|[\'].+?[\']|[\`].+?[\`]|[^ ]+""")]
    private static partial Regex ConsoleArgsRegex();
