...
public class SimpleLogger
{
    ...
    /// </summary>
    private readonly string _version = Assembly.GetExecutingAssembly().GetName().Version?.ToString() ?? "1.0.0.0";

    /// <summary>
    /// Stores the name of the logged app.
    /// </summary>
    public string LoggedAppName { get; set; } = GetTitle();

    /// <summary>
    /// Stores the version of the logged app.
    /// </summary>
    public Version LoggedAppVersion { get; set; } = Assembly.GetEntryAssembly()?.GetName().Version ?? new Version(1, 0, 0, 0);
